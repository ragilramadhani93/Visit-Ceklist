...
const AuditorDashboardView: React.FC<AuditorDashboardViewProps> = ({ user, onSelectChecklist, checklists, tasks, users, onResolveTask }) => {
    ...
    const myChecklists = useMemo(() => checklists.filter(c => c.assigned_to === user.id), [checklists, user.id]);
        ...
        <div className="space-y-6">
            ...
            <Card>
                ...
                <div className="h-3 bg-base-200 rounded">
                    <div className="h-3 bg-success rounded" style={{ width: `${completionRate}%` }} />
                </div>
                <p className="text-sm text-gray-600 mt-2">{completedCount} of {totalCount} checklists completed</p>
            </Card>
